...
class VocabEncoder {
 ...
 public:
  VocabEncoder() {
    ...
    // encode null
    tokenToInt[NULL_SRC_TOKEN_STRING] = NULL_SRC_TOKEN_ID;
    intToToken[NULL_SRC_TOKEN_ID] = NULL_SRC_TOKEN_STRING;
  }
  ...
  int Encode(const std::string& token, bool explicitUseUnk) {
    if(tokenToInt.count(token) == 0) {
      if(explicitUseUnk) {
  ...
  void ReadParallelCorpus(const std::string &textFilename, \
                          ...
                          vector<string> &tgtLattticePaths, \
                          const string &nullToken="") {
     assert(srcSents.size() == 0 && tgtLattticePaths.size() == 0);
     std::ifstream textFile(textFilename.c_str(), std::ios::in);
     std::string line;
     int lineNumber = -1;
     while (getline(textFile, line)) {
        ...
        // data should be in this format: path_to_tgt_lattice_fst ||| source sentence
        std::string tgtFstFilename, det, srcSent;
        tgtFstFilename = splits[0];
        det = splits[1];
        assert(det == "|||");
        srcSents.resize(lineNumber + 1);
        tgtLattticePaths.push_back(tgtFstFilename);
