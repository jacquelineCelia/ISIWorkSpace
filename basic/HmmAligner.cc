...
HmmAligner::HmmAligner(const string& _bitextFilename,
    ...
    if (use_lattice) {
        ...
        FstUtils::LoadLatticeFsts(tgtLatticePaths, tgtSents, tgtFsts, vocabEncoder, false);
    }
    else {
        vocabEncoder.ReadParallelCorpus(_bitextFilename, \
                                        srcSents, \
...
HmmAligner::HmmAligner(const string& tParamsFile, 
   ...
   vocabEncoder.ReadParallelCorpus(testFile,
                                   ...
                                   NULL_SRC_TOKEN_STRING);
   assert(srcSents.size() > 0 && srcSents.size() == tgtLatticePaths.size());
   FstUtils::LoadLatticeFsts(tgtLatticePaths, tgtSents, tgtFsts, vocabEncoder, true, K);
   CreatePerSentGrammarFsts();
}
...
void HmmAligner::InitJustAParams() {
    for(int sentId = 0; sentId < srcSents.size(); sentId++) {  
        ...
        for (int k = 0; k < srcTokens.size(); ++k) {
            if (aFractionalCounts[-1].count(k) == 0) {
                aFractionalCounts[-1][k] = FstUtils::nLog(1/3.0);
            }
            /*
            else {
                aFractionalCounts[-1][k] = Plus(FstUtils::LogWeight(aFractionalCounts[-1][k]), FstUtils::LogWeight(FstUtils::nLog(1/3.0))).Value();   
            }
            */
        }
        /*
...
double HmmAligner::FindLogMax(vector<double>& log_probs) {
...
double HmmAligner::LogSum(vector<double>& array) {
...
void HmmAligner::NormalizePartialParams(unsigned int n, vector<vector<double> >& prob) {
    for (int i = 0; i < n; ++i) {
        prob[0][i] = -aFractionalCounts[0][i]; 
    }
    ...
    for (int i = 1; i < n; ++i) {
        for (int j = 1; j < n; ++j) {
            prob[i][j] = -aFractionalCounts[i][j]; 
        }
        prob[i][0] = prob[i][i];
        elems = prob[i];
...
void HmmAligner::Create1stOrderSrcFst(const vector<int>& srcTokens, VectorFst<FstUtils::LogQuadArc>& srcFst) {
    ...
    for (size_t i = 0; i < srcTokens.size(); ++i) {
        vector<double> zero(srcTokens.size(), 0);
        normalized_prob.push_back(zero);
    }
    NormalizePartialParams(srcTokens.size(), normalized_prob);
    for(int i = 0; i < srcTokens.size(); i++) {
        if(i == 0) {
            srcFst.SetStart(i);
        } 
        ...
        // srcFst.AddArc(i, FstUtils::LogQuadArc(srcTokens[0], srcTokens[0], FstUtils::EncodeQuad(0, i, prevAlignment, aFractionalCounts[prevAlignment][i]), i));
        srcFst.AddArc(i, FstUtils::LogQuadArc(srcTokens[0], srcTokens[0], FstUtils::EncodeQuad(0, i, prevAlignment, normalized_prob[prevAlignment][i]), i));
        for(int j = 1; j < srcTokens.size(); j++) {
            // srcFst.AddArc(i, FstUtils::LogQuadArc(srcTokens[j], srcTokens[j], FstUtils::EncodeQuad(0, j, prevAlignment, aFractionalCounts[prevAlignment][j]), j));
            srcFst.AddArc(i, FstUtils::LogQuadArc(srcTokens[j], srcTokens[j], FstUtils::EncodeQuad(0, j, prevAlignment, normalized_prob[prevAlignment][j]), j));
        }
    }
    ArcSort(&srcFst, ILabelCompare<FstUtils::LogQuadArc>());
}
...
void HmmAligner::LearnParameters() {
    ...
    do {
        ...
        if (use_lattice) {
            stringstream outputAlignmentsFilename;
            outputAlignmentsFilename << outputPrefix << "." << learningInfo.iterationsCount << ".align";
            AlignWithLattice(outputAlignmentsFilename.str());
        } 
        else {
            stringstream outputAlignmentsFilename;
            outputAlignmentsFilename << outputPrefix << "." << learningInfo.iterationsCount << ".align";
            Align(outputAlignmentsFilename.str());
        }
...
void HmmAligner::AlignWithLattice(const string& alignmentsFilename) {
    ...
    for(unsigned sentId = 0; sentId < srcSents.size(); sentId++) {
        ...
        alignmentsLine = AlignSentWithLattice(srcSent, tgtSent, tgtFst);
        outputAlignments << alignmentsLine;
    }
    outputAlignments.close();
}

string HmmAligner::AlignSentWithLattice(vector<int> srcTokens, 
