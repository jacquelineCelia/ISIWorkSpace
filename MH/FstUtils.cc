...
int FstUtils::GetLongestPathLen(fst::VectorFst<fst::LogArc>& fst) {
    fst::VectorFst<fst::StdArc> counter;
    fst::VectorFst<fst::StdArc> longest_path;
    Map(fst, &counter, LogToStdMapper());
    MutableArcIterator<fst::VectorFst<fst::StdArc> > aiter(&counter, counter.Start());
    for (; !aiter.Done(); aiter.Next()) {
        fst::StdArc arc = aiter.Value();
        arc.weight = -1; 
        aiter.SetValue(arc);
    }
    ShortestPath(counter, &longest_path);
    return longest_path.NumStates() - 1; 
}
   ...
   while (!siter.Done()) {
      ...
      while (!aiter.Done()) {
         ...
         tgtFst.AddArc(siter.Value(), 
              FstUtils::LogArc(newValue, newValue, 0, aiter.Value().nextstate)); 
         aiter.Next();
      }
      siter.Next();
   }
...
void FstUtils::LoadLatticeFsts(vector<string>& lattice_paths, \
                                vector<vector<int> >& src_sents, \
                                vector<fst::VectorFst<fst::LogArc> >& prior_fsts, \
                                VocabEncoder& vocabEncoder) {
    vector<string>::iterator siter = lattice_paths.begin();
    for (; siter != lattice_paths.end(); ++siter) {
        vector<int> src_sent;
        fst::VectorFst<fst::LogArc> prior_fst;
        LoadLatticeFst(*siter, src_sent, prior_fst, vocabEncoder);
        src_sents.push_back(src_sent);
        prior_fsts.push_back(prior_fst);
    }
}

void FstUtils::LoadLatticeFst(const string lattice_path, \
                              vector<int>& src_sent, 
                              fst::VectorFst<fst::LogArc>& prior_fst, \
                              VocabEncoder& vocabEncoder) {
   fst::StdVectorFst *input = fst::StdVectorFst::Read(lattice_path);
   const SymbolTable *isyms = input -> InputSymbols();
   StateIterator<StdVectorFst> siter(*input);
   ...
   while (!siter.Done()) {
      ...
      while (!aiter.Done()) {
         ...
         string symbol = isyms -> Find(arcValue);
         int newValue = vocabEncoder.Encode(symbol, false);
         src_sent.push_back(newValue);
         prior_fst.AddArc(siter.Value(), fst::LogArc(newValue, newValue, aiter.Value().weight.Value(), aiter.Value().nextstate));
         aiter.Next();
      }
      siter.Next();
   }
   prior_fst.SetFinal(currentState, fst::LogWeight::One());
   // add __null__ at the beginning of a fst
   int new_start_state = prior_fst.AddState();
   prior_fst.AddArc(new_start_state, \
           fst::LogArc(NULL_SRC_TOKEN_ID, NULL_SRC_TOKEN_ID, fst::LogWeight::One(), input -> Start()));
   src_sent.push_back(NULL_SRC_TOKEN_ID);
   prior_fst.SetStart(new_start_state);
   // top sort the final fst
   TopSort(&prior_fst);
   ArcSort(&prior_fst, ILabelCompare<fst::LogArc>());
}
