...
def main():
   ...
   parser.add_option("--input_dir", dest = "input_dir", help = "directory of input morpheme lattice fst")
   parser.add_option("--output_dir", dest = "output_dir", help = "directory of output sentence lattices")
   (options, args) = parser.parse_args()
   if len(args) != 0:
      parser.error("incorrect number of arguments, please type -h for more help.")

   fin = open(options.text)
   line = fin.readline()
   while line:
      process_a_sentence(line.strip(), options.input_dir, options.output_dir)
      line =fin.readline()
